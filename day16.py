...
logger.add("day16.log", level="DEBUG")
...
if PART1:
    ...
    while not stack.empty():
        ...
        closed_valves = {
            v for v in valves if valves[v].pressure > 0 and v not in opened
        }
...
while stack2:
    ...
    n_paths += 1
    if n_paths % 100000 == 0:
        logger.debug(f"Checked {n_paths} paths")
        if n_paths >= 1000000:
            logger.debug(f"Stack size: {stack2.qsize()}")
    ...
    # items with low priority are checked first
    prio = 1000 - timeleft - new_total - sub
    ...
    for p1, p2 in zip((mypos, elpos), (elpos, mypos)):
        ...
        # p2 walks through tunnel
        for tunnel in valves[p2].tunnels:
            ...
            new_open = opened.copy()
            # (prio, mypos, elpos, opened, sub, total, timeleft)
            stack2.put((prio, p1, p2, new_open, sub, new_total, timeleft - 1))

    # possibility 4: both walk through a tunnel
    for tunnel1 in valves[mypos].tunnels:
        # check if this is a dead end with only open valves
        if is_dead_end(source=mypos, target=tunnel1, opened=tuple(opened)):
            continue

        # if the only tunnel from this valve is the one we came from, and the valve is
        # not open, we don't go back without opening it
        if is_dead_end(tunnel1, mypos, tuple(opened)) and tunnel1 in opened:
            continue

        for tunnel2 in valves[elpos].tunnels:
            # check if this is a dead end with only open valves
            if is_dead_end(source=elpos, target=tunnel2, opened=tuple(opened)):
                continue

            # if the only tunnel from this valve is the one we came from, and the valve is
            # not open, we don't go back without opening it
            if is_dead_end(tunnel2, elpos, tuple(opened)) and tunnel2 in opened:
                continue

            new_open = opened.copy()
            # (prio, mypos, elpos, opened, sub, total, timeleft)
            stack2.put((prio, tunnel1, tunnel2, new_open, sub, new_total, timeleft - 1))

logger.info(f"Checked {n_paths} paths")
logger.debug(f"Found after {best_npaths} paths")

logger.success(f"Best ({best_solution2}): {'-'.join(best_path)}")
